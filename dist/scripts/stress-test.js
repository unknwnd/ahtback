...
// Отправка транзакции
async function sendTransaction(client, walletContract, keyPair, destinationAddress, amount, message = '', queryId = Math.floor(Math.random() * 1000000000)) {
    console.log(`Sending ${amount} nanoTON to ${destinationAddress.toString()}`);
...
async function stressTestFundsDistributor(client, walletContract, keyPair, fundsDistributorAddress) {
    ...
    for (let i = 0; i < NUM_TRANSACTIONS; i++) {
        ...
        try {
            const success = await sendTransaction(client, walletContract, keyPair, fundsDistributorAddress, TRANSACTION_AMOUNT, `Стресс-тест: транзакция ${i + 1}`);
            if (success) {
                successCount++;
            }
            else {
...
async function stressTestAnimalHelperToken(client, walletContract, keyPair, tokenAddress) {
    console.log('\n=== Стресс-тест контракта AnimalHelperToken ===');
    // Статистика
    let successCount = 0;
    let failCount = 0;
    const startTime = Date.now();
    // Создаем специальное сообщение для покупки токенов
    ...
    for (let i = 0; i < NUM_TRANSACTIONS; i++) {
        ...
        try {
            ...
            if (success) {
                ...
                console.log(`Transaction may not have been confirmed.`);
                failCount++;
            }
        }
        catch (error) {
...
async function main() {
    try {
        ...
        const fdResults = await stressTestFundsDistributor(client, walletContract, keyPair, fundsDistributorAddress);
        const tokenResults = await stressTestAnimalHelperToken(client, walletContract, keyPair, tokenAddress);
        // Общие результаты
