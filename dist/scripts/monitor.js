...
(0, dotenv_1.config)({ path: path_1.default.resolve(__dirname, `../${ENV_FILE}`) });
// Путь для сохранения логов
const LOGS_DIR = path_1.default.resolve(__dirname, '../logs');
if (!fs_1.default.existsSync(LOGS_DIR)) {
    fs_1.default.mkdirSync(LOGS_DIR);
}
...
const LARGE_TX_THRESHOLD = (0, core_1.toNano)('1');
...
async function loadDeployInfo() {
    const deployInfoPath = process.argv.includes('--mainnet')
        ? path_1.default.resolve(__dirname, '../deploy-info-mainnet.json')
        : path_1.default.resolve(__dirname, '../deploy-info.json');
    if (!fs_1.default.existsSync(deployInfoPath)) {
        throw new Error(`Deploy info file not found at ${deployInfoPath}`);
    }
    const deployInfo = JSON.parse(fs_1.default.readFileSync(deployInfoPath, 'utf-8'));
    return deployInfo;
}
...
function logWithTimestamp(message, type = 'info') {
    ...
    const logFile = path_1.default.join(LOGS_DIR, `monitor_${new Date().toISOString().split('T')[0]}.log`);
    fs_1.default.appendFileSync(logFile, `${timestamp} ${logPrefix} ${message}\n`);
}
...
async function getBalance(address) {
    ...
    try {
        const balance = await client.getBalance(core_1.Address.parse(address));
        return balance;
    }
    catch (error) {
...
async function checkNewTransactions(contractType, address) {
...
async function downloadVisualizer() {
    ...
    const outputPath = path_1.default.join(LOGS_DIR, 'visualizer.html');
    try {
        const response = await axios_1.default.get(visualizerUrl, { responseType: 'stream' });
        await streamPipeline(response.data, (0, fs_2.createWriteStream)(outputPath));
...
async function getContractsState(deployInfo) {
