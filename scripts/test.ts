...
// Примечание: после компиляции контрактов, импортируйте их из build
// Ниже используются заглушки для демонстрации
interface ContractExecutor {
    getState(): Promise<any>;
}

interface Contract {
    executor?: (provider: any) => ContractExecutor;
}
...
async function testBuyTokens(blockchain: Blockchain, user: SandboxContract<TreasuryContract>) {
    console.log('  Buying tokens...');
    
    // Имитация покупки токенов (отправка TON)
    const buyAmount = toNano('1'); // 1 TON
...
async function testVoting(blockchain: Blockchain, 
                         user1: SandboxContract<TreasuryContract>,
                         user2: SandboxContract<TreasuryContract>,
                         charity: SandboxContract<TreasuryContract>) {
...
async function testNFTMinting(blockchain: Blockchain, user: SandboxContract<TreasuryContract>) {
    console.log('  Testing NFT minting...');
    
    // Имитация покупки большого количества токенов
    const buyAmount = toNano('10'); // 10 TON
    console.log(`  User sends ${buyAmount} TON to buy tokens`);
...
async function testNFTRedemption(blockchain: Blockchain, user: SandboxContract<TreasuryContract>) {
...
function buildOnchainMetadata(params: any): Cell {
