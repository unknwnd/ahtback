...
export function loadBuyTokens(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 1461950307) { throw Error('Invalid prefix'); }
    return { $$type: 'BuyTokens' as const };
}
...
export type WithdrawRemaining = {
    $$type: 'WithdrawRemaining';
    queryId: bigint;
}

export function storeWithdrawRemaining(src: WithdrawRemaining) {
    return (builder: Builder) => {
        const b_0 = builder;
...
export function loadWithdrawRemaining(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 452244840) { throw Error('Invalid prefix'); }
    const _queryId = sc_0.loadIntBig(257);
    return { $$type: 'WithdrawRemaining' as const, queryId: _queryId };
}

function loadTupleWithdrawRemaining(source: TupleReader) {
    const _queryId = source.readBigNumber();
    return { $$type: 'WithdrawRemaining' as const, queryId: _queryId };
}

function loadGetterTupleWithdrawRemaining(source: TupleReader) {
    const _queryId = source.readBigNumber();
    return { $$type: 'WithdrawRemaining' as const, queryId: _queryId };
}

function storeTupleWithdrawRemaining(source: WithdrawRemaining) {
    const builder = new TupleBuilder();
    builder.writeNumber(source.queryId);
    return builder.build();
}

function dictValueParserWithdrawRemaining(): DictionaryValue<WithdrawRemaining> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeWithdrawRemaining(src)).endCell());
        },
        parse: (src) => {
...
export function storeUpdateRate(src: UpdateRate) {
    return (builder: Builder) => {
        const b_0 = builder;
...
export function loadUpdateRate(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 2397651930) { throw Error('Invalid prefix'); }
    const _queryId = sc_0.loadIntBig(257);
    const _newRate = sc_0.loadIntBig(257);
    return { $$type: 'UpdateRate' as const, queryId: _queryId, newRate: _newRate };
}

function loadTupleUpdateRate(source: TupleReader) {
    const _queryId = source.readBigNumber();
    const _newRate = source.readBigNumber();
    return { $$type: 'UpdateRate' as const, queryId: _queryId, newRate: _newRate };
}

function loadGetterTupleUpdateRate(source: TupleReader) {
    const _queryId = source.readBigNumber();
    const _newRate = source.readBigNumber();
    return { $$type: 'UpdateRate' as const, queryId: _queryId, newRate: _newRate };
}

function storeTupleUpdateRate(source: UpdateRate) {
    const builder = new TupleBuilder();
    builder.writeNumber(source.queryId);
...
function dictValueParserUpdateRate(): DictionaryValue<UpdateRate> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeUpdateRate(src)).endCell());
        },
        parse: (src) => {
...
export function storeTokenSale$Data(src: TokenSale$Data) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeAddress(src.jettonMaster);
...
export function loadTokenSale$Data(slice: Slice) {
    const sc_0 = slice;
    const _owner = sc_0.loadAddress();
    const _jettonMaster = sc_0.loadAddress();
    ...
    return { $$type: 'TokenSale$Data' as const, owner: _owner, jettonMaster: _jettonMaster, jettonWallet: _jettonWallet, fundsDistributor: _fundsDistributor, tokenRate: _tokenRate, isActive: _isActive };
}

function loadTupleTokenSale$Data(source: TupleReader) {
    const _owner = source.readAddress();
    const _jettonMaster = source.readAddress();
...
function loadGetterTupleTokenSale$Data(source: TupleReader) {
    const _owner = source.readAddress();
    const _jettonMaster = source.readAddress();
    const _jettonWallet = source.readAddress();
    const _fundsDistributor = source.readAddress();
...
function storeTupleTokenSale$Data(source: TokenSale$Data) {
    const builder = new TupleBuilder();
    builder.writeAddress(source.owner);
    builder.writeAddress(source.jettonMaster);
...
function dictValueParserTokenSale$Data(): DictionaryValue<TokenSale$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeTokenSale$Data(src)).endCell());
        },
        parse: (src) => {
...
export function loadSaleData(slice: Slice) {
    ...
    return { $$type: 'SaleData' as const, jettonWallet: _jettonWallet, fundsDistributor: _fundsDistributor, isActive: _isActive, tokenRate: _tokenRate };
}
...
function loadGetterTupleSaleData(source: TupleReader) {
    const _jettonWallet = source.readAddress();
    const _fundsDistributor = source.readAddress();
    const _isActive = source.readBoolean();
    const _tokenRate = source.readBigNumber();
...
function dictValueParserSaleData(): DictionaryValue<SaleData> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeSaleData(src)).endCell());
        },
        parse: (src) => {
...
function initTokenSale_init_args(src: TokenSale_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeAddress(src.jettonMaster);
...
async function TokenSale_init(owner: Address, jettonMaster: Address, jettonWallet: Address, fundsDistributor: Address, tokenRate: bigint) {
...
export const TokenSale_errors_backward = {
    "Stack underflow": 2,
    "Stack overflow": 3,
    "Integer overflow": 4,
...
export class TokenSale implements Contract {
    
    public static readonly storageReserve = 0n;
    public static readonly errors = TokenSale_errors_backward;
    public static readonly opcodes = TokenSale_opcodes;
    
    static async init(owner: Address, jettonMaster: Address, jettonWallet: Address, fundsDistributor: Address, tokenRate: bigint) {
        return await TokenSale_init(owner, jettonMaster, jettonWallet, fundsDistributor, tokenRate);
    }
    
    static async fromInit(owner: Address, jettonMaster: Address, jettonWallet: Address, fundsDistributor: Address, tokenRate: bigint) {
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: BuyTokens | "toggle_sale" | UpdateRate | WithdrawRemaining | Deploy) {
        ...
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'UpdateRate') {
            body = beginCell().store(storeUpdateRate(message)).endCell();
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'WithdrawRemaining') {
            body = beginCell().store(storeWithdrawRemaining(message)).endCell();
        }
